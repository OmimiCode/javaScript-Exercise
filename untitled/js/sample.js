...
function sandwichCalculator(sliceOfBread){
    const sandwich = Math.trunc(sliceOfBread /2 );
    result = alert("you can only make "+ sandwich + " sandwich from the slice Of bread provided")
    return result;
}


function sandwichWithCheeseCalculator(){
    const sliceOfBread = parseInt(prompt("enter slice of bread available :  "));
    const sliceOfCheese = parseInt(prompt("enter slice of cheese available :  "));
    const sandwich = Math.trunc(sliceOfBread / 2);
